...
export function SchichtplanungDialog({ open, onClose, onSubmit, defaultValues, mitarbeiterList, schichttypenList, enablePhotoScan = true, enablePhotoLocation = true }: SchichtplanungDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="schicht_auswahl" entity="Schichttypen">\n${JSON.stringify(schichttypenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "datum": string | null, // YYYY-MM-DD\n  "mitarbeiter_auswahl": string | null, // Display name from Mitarbeiter (see <available-records>)\n  "schicht_auswahl": string | null, // Display name from Schichttypen (see <available-records>)\n  "abteilung_plan": string | null, // Abteilung\n  "standort": string | null, // Standort / Bereich\n  "status": LookupValue | null, // Status (select one key: "geplant" | "bestaetigt" | "abwesend" | "vertreter") mapping: geplant=Geplant, bestaetigt=Bestätigt, abwesend=Abwesend, vertreter=Vertreter\n  "notiz": string | null, // Notiz\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["mitarbeiter_auswahl", "schicht_auswahl"]);
        ...
        if (schicht_auswahlName) {
          const schicht_auswahlMatch = schichttypenList.find(r => matchName(schicht_auswahlName!, [String(r.fields.schichtname ?? '')]));
          if (schicht_auswahlMatch) merged['schicht_auswahl'] = createRecordUrl(APP_IDS.SCHICHTTYPEN, schicht_auswahlMatch.record_id);
        }
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="schicht_auswahl">Schichttyp</Label>
            <Select
              value={extractRecordId(fields.schicht_auswahl) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, schicht_auswahl: v === 'none' ? undefined : createRecordUrl(APP_IDS.SCHICHTTYPEN, v) }))}
            >
          ...
          <div className="space-y-2">
            <Label htmlFor="abteilung_plan">Abteilung</Label>
            <Input
              id="abteilung_plan"
              value={fields.abteilung_plan ?? ''}
              onChange={e => setFields(f => ({ ...f, abteilung_plan: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="standort">Standort / Bereich</Label>
            <Input
              id="standort"
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="vertreter">Vertreter</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
